...
Configurations inside the window gain **+2.48 pp**; those outside lose 0.31 pp
(Welch t = 8.42, **p < 10⁻⁶**). The reported 45% capability ceiling is reproduced as the window's
**upper** edge — but expert-level medical items reach a
...
**All 30 statistically significant gains lie inside the window**; not one of the 100
configurations outside it reaches significance at any panel size under any architecture. The
largest come from the **attending-physician (Centralized) architecture at N = 3**: +7.6 pp
(p = 2×10⁻⁵) and +6.4 pp (p = 0.0037). Panel size predicts nothing
(β_log(1+n_a) = −0.003, p = 0.70).

## Scale

**180 multi-agent configurations** · 63,499 episodes · 3 capability tiers × 3 medical benchmarks × 5 architectures
× N ∈ {1,3,5,7,9} · OpenAI models only · total API spend **$39**.

## What replicates and what does not

| | NMI 2026 | this work |
|---|---|---|
| gain-model cross-validated R² | 0.513 | **0.415** (no tool term) |
...
Preprint in preparation. All 9 tier×benchmark cells complete (180 multi-agent configurations,
matching the 180 of the reference study). Gemini and Claude families are planned; the figure
code already supports them — register the models in `FAMILY` in `experiments/vizstyle.py`.
